...
const DOMAIN: DomainDiscovery = { slug: "nuxt", techName: "Nuxt", match: "topic:nuxt", keep: 5, exclude: ["nuxt/nuxt"], languageFamilies: [] };
...
test("hard noise (fork / framework core) is never kept, even in a tiny ecosystem", async () => {
  ...
  const gh = new GitHub({ token: "x", etags: memStore() });
  const ds = await scrapeDomain(gh, { slug: "twill", techName: "Twill", match: "twill", keep: 12, exclude: ["area17/twill"], languageFamilies: [] }, Date.now());
  const ids = ds.projects.map((p) => p.githubId);
