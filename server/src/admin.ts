/**
 * Admin API — every route is gated by app-level auth (session cookie / bearer
 * token). Covers the full flow: overview stats, submission moderation, per-entry
 * overrides (hide/feature/edit), and manual refresh (scrape) / republish.
 */
...
import { readRaw, readDataset, writeDomainConfig } from "./storage";
import { kvEtagStore } from "./kv";
import { json, triggerRebuild } from "./util";
...
const ghClient = (env: AppEnv) => new GitHub({ token: env.githubToken, etags: kvEtagStore(env.kv), log: (m) => console.log(m) });

async function galleryProjectCount(store: ObjectStore, slug: string): Promise<number> {
  const [pubText, raw] = await Promise.all([readDataset(store, slug), readRaw(store, slug)]);
  if (pubText) {
    try {
...
export async function handleAdmin(req: Request, env: AppEnv, path: string): Promise<Response> {
  const identity = requireAdmin(req, env);
  ...
  if (seg[0] === "overview" && method === "GET") {
    const [lastRun, pending, domains] = await Promise.all([
      env.kv.getJson("meta:lastRun"),
      db.countPending(),
      Promise.all(DOMAINS.map(async (d) => {
        const [raw, pubText, published] = await Promise.all([
          readRaw(store, d.slug),
          readDataset(store, d.slug),
          galleryProjectCount(store, d.slug),
        ]);
        let pub: any = null;
  ...
  if (seg[0] === "submissions") {
    ...
    if (method === "POST" && seg[1]) {
      ...
      if (body.action === "approve") {
        ...
        const merged = await publishFromRaw(store, db, sub.slug, getDomain(sub.slug)?.keep ?? DEFAULT_KEEP);
        void triggerRebuild(env);
  ...
  if (seg[0] === "domains") {
    if (method === "GET" && !seg[1]) {
      ...
      const domains = await Promise.all(DOMAINS.map(async (d) => {
        ...
        const published = await galleryProjectCount(store, d.slug);
    ...
    if (seg[1]) {
      ...
      if (method === "POST") {
        ...
        await writeDomainConfig(store, slug, { slug, ...merged, updatedAt: nowIso });
        void triggerRebuild(env);
  ...
  if (seg[0] === "entries" && seg[1]) {
    ...
    if (method === "GET" && !seg[2]) {
      const [raw, overrides, approved] = await Promise.all([readRaw(store, slug), db.overridesFor(slug), db.approvedFor(slug)]);
      const ov = new Map(overrides.map((o) => [o.github_id, o]));
      const manualIds = new Set(approved.map((a) => a.githubId));
      const base = [...(raw?.projects ?? []), ...approved];
      const seen = new Set<number>();
    ...
    if (method === "POST" && seg[2]) {
      ...
      const merged = await publishFromRaw(store, db, slug, dcfg.keep);
      void triggerRebuild(env);
  ...
  if ((seg[0] === "refresh" || seg[0] === "republish") && method === "POST") {
    ...
    if (seg[0] === "refresh") {
      if (!env.githubToken?.trim()) {
        return json({ error: "GITHUB_TOKEN is not configured on the server", refreshed: [] }, 503);
      }
      ...
      if (targets.length > 1 || !body.slug) {
        await env.kv.put("meta:lastRun", JSON.stringify({ at: new Date().toISOString(), summary: results }));
      }
      void triggerRebuild(env);
    ...
    for (const d of targets) {
      ...
      const merged = await publishFromRaw(store, db, d.slug, d.keep);
      if (!merged) {
