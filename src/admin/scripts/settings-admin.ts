...
const SITE_GROUPS = [
  {
    ...
    parameters: [
      ...
      },
      {
        ...
        label: "Build data URL",
        required: false,
        purpose: "scripts/pull-data.mjs fetches /data/*.json from the Worker at build time.",
    ...
    parameters: [
      {
        ...
        required: false,
        purpose: "npm run scrape — local catalog refresh into src/data/*.json.",
        configure: "Root .env GITHUB_TOKEN=…",
      },
      {
        key: "SCRAPE_PER_DOMAIN",
        label: "Scrape per domain",
        required: false,
        purpose: "Projects kept per domain in scripts/scrape.mjs (default 100).",
        configure: "Environment variable when running scrape",
      },
    ],
  },
];
