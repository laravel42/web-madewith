 ...
 *
 * Priority:
 * 1. Postgres (DATABASE_URL) — runs workers/projects/publish.py (ranked repos → JSON)
...
const PROJECTS_DIR = join(WORKERS_DIR, "projects");
const VIDEOS_DIR = join(WORKERS_DIR, "videos");
const UTILS_DIR = join(WORKERS_DIR, "utils");
...
function publishPython() {
  const venv = join(WORKERS_DIR, ".venv", "bin", "python");
...
async function hydrateFromPostgres() {
  ...
  const ok = await runPythonScript(join(PROJECTS_DIR, "publish.py"), "projects");
  if (ok) {
    console.log("pull-data: project hydration complete");
    // Same post-publish guards projects:publish runs: evict cross-domain
    // contamination, then preserve/assign per-project addedAt (feeds sort by it).
    await runNodeScript(join(UTILS_DIR, "scrub-cross-domain.mjs"), "scrub");
    await runNodeScript(join(UTILS_DIR, "stamp-added-at.mjs"), "addedAt stamp");
  } else {
...
async function hydrateVideosFromPostgres() {
  ...
  const ok = await runPythonScript(join(VIDEOS_DIR, "publish_videos.py"), "videos");
