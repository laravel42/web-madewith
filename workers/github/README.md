...
```bash
# This package shares workers/.venv with every other worker.
python3 -m venv workers/.venv
workers/.venv/bin/pip install -r workers/requirements.txt
workers/.venv/bin/pip install -e workers/github   # puts `madewith-github` on the venv PATH
...
```bash
workers/.venv/bin/pip install -e "workers/github[dev]"
pytest
```
...
The cron entry points are Python now; the old shell runners are gone.

```bash
pnpm github:discover     # or: workers/.venv/bin/python workers/github/scheduled.py discover
pnpm github:import       # replay published JSON, then qualify only if new repos landed
pnpm github:cli -- --help
```

Both hold an atomic lockfile under `workers/github/logs/`, so a still-running
job makes the next scheduled one skip rather than overlap, and both rotate
their log past 10 MB. **Any LaunchAgent plist must be repointed at
`workers/github/scheduled.py` — the old `github/run_*.sh` paths no longer exist.**
