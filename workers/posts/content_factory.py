...
REPO_ROOT_ENV = BASE_DIR.parent.parent / ".env"
PARENT_ENV = BASE_DIR.parent / ".env"
LOCAL_ENV = BASE_DIR / ".env"

# override=True: this project's own .env is the source of truth for its LLM
# credentials/endpoint. Without it, an already-exported shell variable (e.g. a
# stale OPENAI_API_KEY/OPENAI_BASE_URL left over from routing some other tool
# through OpenRouter) silently wins over .env and never even surfaces as an
# error — the request just goes to the wrong provider with the wrong key.
...
# LLM provider: OpenAI direct whenever OPENAI_API_KEY is set (FACTORY_BASE_URL
# overrides the endpoint), OpenRouter only as a fallback when no OpenAI key is
# configured. The endpoint is always resolved to a concrete URL — never left
# as None — so an ambient OPENAI_BASE_URL env var (e.g. a stale shell export
# pointing at OpenRouter) can't silently hijack an "OpenAI direct" run; the
# openai SDK only consults that env var when base_url is omitted. Model
# precedence: FACTORY_MODEL > backend default (gpt-5-mini on OpenAI;
# Sonnet 4.5 on OpenRouter for long-form editorial quality).
if os.getenv("FACTORY_BASE_URL"):
    LLM_BASE_URL = os.environ["FACTORY_BASE_URL"]
    ...
    LLM_BASE_URL = "https://api.openai.com/v1"
LLM_USE_CHAT = "api.openai.com" not in LLM_BASE_URL
if os.getenv("FACTORY_MODEL"):
    LLM_MODEL = os.environ["FACTORY_MODEL"]
elif "openrouter" in LLM_BASE_URL:
...
def main() -> None:
    ...
    # Prefer the key matching the endpoint (mirrors workers/videos/enrich_transcripts.py):
    ...
    if not llm_key and not args.dry_run:
        die("An LLM key is missing from ../.env or ./.env: set OPENAI_API_KEY (or OPENROUTER_API_KEY)")
