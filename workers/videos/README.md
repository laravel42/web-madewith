...
The repo-root `pnpm pipeline videos` aggregates the whole flow (transcribe →
...
pnpm youtube:scrape -- -a domains=laravel -a refresh_days=0
# or directly:
cd workers/videos && ../../workers/.venv/bin/scrapy crawl youtube -a domains=laravel
```
...
```bash
pnpm youtube:publish            # all domains
pnpm youtube:publish -- laravel # one slug
```
...
time by `workers/utils/pull-data.mjs`.

## Transcripts

`transcribe_youtube.py` reads pending videos from PostgreSQL and writes raw
caption files to the local cache `workers/videos/data/tranworkers/utils/<videoId>.json`
(Git-ignored):

```bash
workers/.venv/bin/python workers/videos/transcribe_youtube.py --slug laravel --limit 20
workers/.venv/bin/python workers/videos/transcribe_youtube.py --limit 500 --sleep 1 --workers 2
...
| `--out` | `workers/videos/data/transcripts` | Raw-caption output directory |
...
```bash
workers/.venv/bin/python workers/videos/enrich_transcripts.py --video-id DKnn8TlJ4MA --force
...
```bash
workers/.venv/bin/python workers/videos/rewrite_descriptions.py --dry-run   # preview queue
workers/.venv/bin/python workers/videos/rewrite_descriptions.py             # rewrite
```
...
```bash
workers/.venv/bin/python workers/videos/cleanup_videos.py                 # dry run — report only
workers/.venv/bin/python workers/videos/cleanup_videos.py --slug phoenix  # scope to one slug
workers/.venv/bin/python workers/videos/cleanup_videos.py --apply         # delete explicit off-topic rejects
workers/.venv/bin/python workers/videos/cleanup_videos.py --aggressive --apply   # also delete missing-signal failures
```
