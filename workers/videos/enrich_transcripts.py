...
Raw caption files live in workers/videos/data/transcripts/<videoId>.json. Published,
...
def main() -> int:
    ...
    parser.add_argument("--base-url",
                        help="OpenAI-compatible endpoint override, e.g. http://localhost:11434/v1 "
                             "for Ollama (default: OpenAI direct, or OpenRouter when no "
                             "OPENAI_API_KEY is set). Ignores the ambient OPENAI_BASE_URL env "
                             "var — pass this flag explicitly.")
    ...
    if args.dry_run:
        ...
        return 0
    # OpenAI direct is the default whenever OPENAI_API_KEY is set; OPENROUTER_API_KEY
    # only kicks in as a fallback when there's no OpenAI key — and only via this
    # explicit check, never via an ambient OPENAI_BASE_URL env var (an explicit
    # --base-url still wins, for a real local server).
    ...
    use_chat = "api.openai.com" not in args.base_url
    # Prefer the key matching the endpoint: OPENAI_API_KEY can then stay in
    # .env for other tools (workers/posts/content_factory.py, github LLM classifier)
