...
def _load_rewrites() -> dict[str, str]:
    """LLM-rewritten descriptions from workers/videos/rewrite_descriptions.py."""
...
def main() -> None:
    ...
    try:
        ...
        for domain in domains:
            ...
            # scrapedAt only advances when videos actually changed — otherwise
            # every no-op publish re-stamps "now" across every domain's video
            # RSS <lastBuildDate>, making a content-identical deploy look
            # changed to deploy_s3.py's ETag diff and firing a CDN
            # invalidation for nothing (mirrors workers/projects/publish.py).
            scraped_at = datetime.now(timezone.utc).isoformat()
            try:
                previous = json.loads(out.read_text(encoding="utf-8"))
                if previous.get("videoCount") == len(relevant) and previous.get("videos") == relevant:
