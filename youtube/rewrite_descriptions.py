...
Provider: OpenRouter when OPENROUTER_API_KEY is set (default model
google/gemini-2.5-flash — short rewrites don't need a frontier model),
OpenAI direct via OPENAI_API_KEY otherwise (gpt-5-mini). Override with
DESC_MODEL or --model; --base-url/OPENAI_BASE_URL for any other
...
def main() -> int:
    ...
    parser.add_argument("--base-url", default=os.getenv("OPENAI_BASE_URL"),
                        help="OpenAI-compatible endpoint (defaults to OpenRouter when "
    ...
    on_openrouter = bool(args.base_url) and "openrouter" in args.base_url
    if on_openrouter:
    ...
    if not api_key and not is_local:
        raise SystemExit("An API key is required: OPENROUTER_API_KEY or OPENAI_API_KEY")
    if args.model:
        model = args.model
    ...
    print("Re-run publish to apply: youtube/.venv/bin/python youtube/publish_videos.py")
