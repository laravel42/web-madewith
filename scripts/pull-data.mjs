...
function valid(payload) {
  return payload && typeof payload.slug === "string" && Array.isArray(payload.projects) && payload.projects.length > 0;
}
...
async function main() {
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      if (!valid(payload)) throw new Error("invalid payload shape");
  ...
  console.log(`pull-data: ${updated}/${slugs.length} refreshed from R2.`);
}
